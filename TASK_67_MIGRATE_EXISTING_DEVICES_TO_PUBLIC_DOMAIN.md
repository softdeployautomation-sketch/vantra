...
**Exclusion (owner instruction, 2026-09-21): devices owned by `myrate619@gmail.com` or `mymood619@gmail.com` are NOT part of this migration — they stay on `api.instaweb.top`.** These are the two exempt accounts that will eventually get real private orgs (Task 60's admin-grant button, once Task 66 is done) — `api.instaweb.top` is where their traffic is SUPPOSED to end up, so migrating their devices to public now and back to private later would just be unnecessary churn. **This specifically includes real customer devices under a user's FIRST org** — e.g. `myrate619@gmail.com`'s `Sc01t` org has a genuine customer device (`WilkSF9`) that must be excluded from this migration exactly like any other of their devices; it does not get different treatment just because it happens to sit on their oldest org.

**Resolve the exclusion by a real DB join (agent → TRMM client → org → `Organization.ownerId` → `User.email`), never by pattern-matching the TRMM client-name slug against an org's OWN id — that check is wrong for a large share of real orgs and produces false negatives.** Concretely, live 2026-09-21: I made exactly this mistake while triaging this task by hand — `clientNameFor()` (`lib/provision.ts`) uses TWO different slug shapes depending on org age, and searching for an org's own id in the slug only catches the second one:
- A user's **first** org keeps the **legacy** slug `vantra-{userId}` — the **user's** id, not the org's.
- Every **additional** org for that user gets a **two-axis** slug `vantra-{userId}-{orgId}` — both ids present.

Searching a client-name slug for an org's own id therefore silently misses every "first org" case — `Sc01t` (myrate's first org, holding the real `WilkSF9` device) looked "orphaned" under that check purely because its slug is `vantra-cmtiz6bva0001kpzw66j2n4z4` (myrate's *user* id) with no org id in it at all, when in fact it was never orphaned — the check was wrong, not the data. The one reliable way to tell "does this org actually own this TRMM client" is `Organization.trmmClientId` matched against the agent's real numeric TRMM client id — resolve that server-side (fetch each agent's client id from TRMM, not the display slug) and join against Vantra's own `Organization.trmmClientId` column. Never infer ownership from the human-readable slug text.

For every currently-registered TRMM agent belonging to a `"public"`-tier organization, EXCLUDING the two owners above:
